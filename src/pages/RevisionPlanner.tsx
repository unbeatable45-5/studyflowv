...
const RevisionPlanner = () => {
  ...
  const handleGenerate = async () => {
    ...
    if (validCourses.length === 0 || loading) return;
    setLoading(true);
    setOutput("");
    ...
    await streamAI({
      ...
      onDone: () => { setLoading(false); saveOutput("revision-planner", { courses: courseData, hoursPerDay: hours[0] }, fullText); },
    ...
    <div className="px-4 py-6 max-w-lg mx-auto space-y-5">
      <div className="space-y-1">
        <div className="flex items-center gap-2">
          <div className="bg-warning/10 rounded-lg p-2"><CalendarDays className="h-5 w-5 text-warning" /></div>
      ...
      <div className="space-y-3">
        ...
        {courses.map((course, i) => (
          <Card key={i} className="p-3 space-y-3">
            <div className="flex gap-2 items-center">
              <Input placeholder="Course name" value={course.name} onChange={(e) => updateCourse(i, { name: e.target.value })} disabled={loading} className="flex-1" />
              {courses.length > 1 && (
                <Button variant="ghost" size="icon" onClick={() => removeCourse(i)} className="shrink-0"><X className="h-4 w-4" /></Button>
              )}
            </div>
            <div className="flex items-center gap-3">
              <div className="flex items-center gap-2">
                <Checkbox id={`unknown-${i}`} checked={course.unknownDate} onCheckedChange={(checked) => updateCourse(i, { unknownDate: !!checked, examDate: checked ? undefined : course.examDate })} />
                <Label htmlFor={`unknown-${i}`} className="text-sm text-muted-foreground">Date unknown</Label>
              </div>
              {!course.unknownDate && (
                <Popover>
                  <PopoverTrigger asChild>
                    <Button variant="outline" size="sm" className={cn("text-xs", !course.examDate && "text-muted-foreground")}>
                      <CalendarDays className="h-3.5 w-3.5 mr-1" />
                      {course.examDate ? format(course.examDate, "MMM d, yyyy") : "Exam date"}
                    </Button>
                  </PopoverTrigger>
                  <PopoverContent className="w-auto p-0" align="start">
                    <Calendar mode="single" selected={course.examDate} onSelect={(date) => updateCourse(i, { examDate: date })} className="p-3 pointer-events-auto" />
                  </PopoverContent>
                </Popover>
              )}
            </div>
        ...
        <Button variant="outline" size="sm" onClick={addCourse} className="gap-1.5"><Plus className="h-4 w-4" /> Add Course</Button>
      </div>
